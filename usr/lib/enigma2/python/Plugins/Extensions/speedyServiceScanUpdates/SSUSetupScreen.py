...
class SSUSetupScreen(ConfigListScreen, Screen):
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['ColorActions', 'OkCancelActions'], {
            'red': self.cancel,  # Abbrechen Methode
            'green': self.save,  # Speichern Methode
            'yellow': self.restore_default,  # Restore Default
            'blue': self.openUpdate,  # Update-Screen öffnen
            'help': self.updateHelp,  # EPG-Taste auf die updateHelp-Methode legen
            'ok': self.save,
            'cancel': self.cancel
        }, -1)
    ...
    def populateList(self):
        ...
        # Direkt den helpText zuweisen
        for entry in self.list:
            entry[1].helpText = entry[2]  # Hilfetext aus der dritten Spalte übernehmen
    ...
    def update_installed_callback(self, choice):
        """Callback nach der Zustimmung oder Ablehnung des Updates"""
        if choice:
            try:
                # Import sicherstellen
                from SSUUpdateScreen import SSUUpdateScreen  # Hier könnte auch der absolute Import stehen
                
                # Update-Screen instanziieren
                screen = SSUUpdateScreen(self.session)
                
                # Bildschirm öffnen
                self.session.open(screen)
            except Exception as e:
                print(f"Error opening update screen: {e}")
        else:
...
class SSUHelpScreen(Screen):
    ...
    def _screen_size(self):
        ...
        try:
            ...
            return 1280, 720  # Standardwerte bei Fehler
