...
class SSUUpdateScreen(Screen):
    ...
    def showHelp(self):
        self.session.open(MessageBox, _("Press GREEN to start update, BLUE to check for updates, RED to exit."), MessageBox.TYPE_INFO, 8)

    def showHelpInfo(self):
        self.session.open(MessageBox, _("This screen lets you manage the Service Scan Updates plugin."), MessageBox.TYPE_INFO, 8)
...
class SSUSetupScreen(ConfigListScreen, Screen):
    def __init__(self, session):
        ...
        if w >= 1920:
            ...
            """<screen name="SSUSetupScreen" position="center,170" size="1000,820" title="speedy Service Scan Setup" backgroundColor="black">
    <ePixmap pixmap="skin_default/buttons/red.png" position="10,5" size="5,70" scale="stretch" alphatest="on" />
    ...
    def populateList(self):
        self.list = [
            ...
            getConfigListEntry(_("Clear bouquet at each search"), config.plugins.speedyservicescanupdates.clear_bouquet, _("Empty the 'Service Scan Updates' bouquet on every scan, otherwise new services will be appended?"))
        ]
